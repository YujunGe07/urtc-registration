...
async function snapshot(db: Database) {
  ...
  return {
    ...
    state: data.data as SchedulerState,
  };
}
async function actorFor(request: Request, db: Database): Promise<PortalActor> {
  let email = '',
    signedIn = false,
    organizer = false;
  // Temporary email-only entry: intentionally checks membership, not email ownership.
  // Remove this branch and organizer-login when restoring verified authentication.
  const claimedEmail = (request.headers.get('x-organizer-email') ?? '')
    .trim()
    .toLowerCase();
  if (claimedEmail && claimedEmail.length <= 254) {
    const allowed = await db
      .from('urtc_organizers')
      .select('email')
  ...
  if (jwt && !organizer) {
    ...
    if (!error && user?.email && user.email_confirmed_at) {
      ...
      const allowed = await db
        ...
        .maybeSingle();
      if (allowed.error) throw allowed.error;
      organizer = Boolean(allowed.data);
    }
  }
...
export async function handle(request: Request): Promise<Response> {
  ...
  if (origin && allowedOrigins.has(origin))
    Object.assign(cors, {
      ...
      'Access-Control-Allow-Headers':
        'authorization,apikey,content-type,x-client-info,x-presenter-session,x-organizer-email',
  ...
  try {
    ...
    if (request.method === 'GET')
      return json(visibleData(await snapshot(db), await actorFor(request, db)));
    if (!request.headers.get('content-type')?.includes('application/json'))
      throw new PortalError('JSON is required.', 415);
    ...
    if (!action || typeof action !== 'object' || Array.isArray(action))
      throw new PortalError('Invalid request.');
    if (action.kind === 'organizer-login') {
      const actor = await actorFor(request, db);
      if (!actor.organizer)
        throw new PortalError(
          'This email address is not on the approved organizer list.',
          403,
        );
      return json(visibleData(await snapshot(db), actor));
    }
    ...
    if (action.kind === 'login') {
      const ip =
        ...
        'unknown';
      const { data: attempts, error: rateError } = await db.rpc(
        'urtc_count_login',
        {
          p_id: `${await hash(ip)}:${Math.floor(Date.now() / 900000)}`,
          p_expiry: Date.now() + 900000,
        },
      );
      ...
      const submission = current.state.submissions.find((s) => s.key === key);
      if (!submission)
        throw new PortalError(
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      if (actor.organizer && action.revision !== current.revision)
        throw new PortalError(
          'The conference changed. Refresh and review before saving again.',
          409,
        );
      const next = applyAction(current.state, action, actor);
      const { data, error } = await db
        .from('urtc_conferences')
        .update({ data: next, revision: current.revision + 1 })
        .eq('id', 'urtc-2027')
      ...
      if (data)
        return json(
          visibleData({ state: next, revision: Number(data.revision) }, actor),
        );
    }
    ...
    return json(
      {
        error:
          'The scheduling service is unavailable. Your changes have not been saved. Please try again.',
      },
      503,
    );
  }
